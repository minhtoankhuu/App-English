...
- [x] **Sinh thiếu câu (mỗi kiểu 1 câu thay vì đủ số yêu cầu) + gạch chân thừa đuôi -s/-es**: liệt kê "(1)(2)(3)" ở mục Pronunciation vừa thêm khiến model hiểu nhầm thành checklist "làm đủ cả 3 kiểu" thay vì "mỗi câu độc lập chọn 1 kiểu" — đặt 5 câu chỉ ra 3 (chủ dự án phát hiện qua dùng thử thật). Thêm câu nhấn mạnh tường minh ở cả system prompt chung lẫn đầu hướng dẫn Pronunciation; thêm ví dụ sai/đúng cho phạm vi gạch chân đuôi -s/-es và -ed (chỉ bọc đúng đuôi). `PROMPT_VERSION` → v4.
- [x] **Fix v4 hiểu ngược ý chủ dự án — sửa lại v5**: "mỗi câu độc lập chọn 1 kiểu" (v4) khiến 1 lần sinh trộn lẫn cả 3 kiểu Pronunciation trong cùng 1 loạt câu — chủ dự án gửi ảnh chụp thật cho thấy hậu quả: nhảy đề tài liên tục, có câu bỏ trống câu dẫn, dùng cụm từ ("computer games") thay vì từ đơn, trùng lựa chọn ("interested" x2), thiếu gạch chân ở một số lựa chọn. Ý đúng: 1 lần sinh CHỈ dùng đúng 1 kiểu cho toàn bộ số câu yêu cầu — muốn nhiều kiểu thì tạo nhiều lượt/phần riêng. Viết lại theo hướng này, thêm ràng buộc 4 lựa chọn phải khác nhau + cả 4 lựa chọn (kể cả đáp án sai) đều phải có gạch chân, áp dụng luôn cho "stress". `PROMPT_VERSION` → v5.
- [x] **Tick "Pronunciation" tạo sẵn 1 block "PRONUNCIATION" + 3 Phần con (1/2/3)**: vì 3 kiểu không trộn được trong 1 lần sinh, muốn đủ cả 3 kiểu (15 câu) giáo viên trước đó phải tự tạo/cấu hình thêm. Lần thử đầu tách thành 3 block riêng (I/II/III độc lập) — sau khi xem kết quả thật, chủ dự án đổi ý: đúng ra phải là 1 khối chung "I. PRONUNCIATION" với 3 phần con đánh số 1/2/3 bên trong (khớp cấu trúc đề tham khảo đã gửi trước đó), không phải 3 mục lớn tách rời. `handleToggleType` (`ExamBuilderPage.tsx`) đặc cách `exercise_type.code === "pronunciation"`: tạo 1 block rồi thêm 3 Phần con (5 câu/phần, mỗi phần ghim đúng 1 kiểu qua `prompt_override`) — tái dùng nguyên cơ chế Phần con + tự đồng bộ `question_count` đã có sẵn từ trước, không cần đổi backend.
