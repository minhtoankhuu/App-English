...
- [x] **Tick "Pronunciation" tạo sẵn 3 block riêng thay vì 1 block 5 câu**: vì 3 kiểu không trộn được trong 1 lần sinh, muốn đủ cả 3 kiểu (15 câu) giáo viên trước đó phải tự tạo/cấu hình thêm — chủ dự án chọn không dùng cơ chế "Phần con" có sẵn (đòi hỏi tự thêm/cấu hình từng phần) mà muốn tick 1 lần ra sẵn 3 block độc lập. `handleToggleType` (`ExamBuilderPage.tsx`) đặc cách `exercise_type.code === "pronunciation"`: tạo tuần tự 3 block (5 câu/block, tự đánh số I/II/III), mỗi block ghim đúng 1 kiểu qua `prompt_override`. Bỏ tick xóa cả 3 — tái dùng nguyên logic `blocksOfType` (filter theo `exercise_type.id`) đã hỗ trợ nhiều block cùng dạng từ trước, không cần đổi backend.
