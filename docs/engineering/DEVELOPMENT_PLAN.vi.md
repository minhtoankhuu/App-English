...
- [x] Adapter provider thật OpenAI + RAG đầy đủ (nhánh `feat/1d-openai-rag-integration`, xem `docs/superpowers/specs/2026-07-21-llm-rag-integration-design.md`): chủ dự án chọn OpenAI (đã có API key) và quyết định làm trọn RAG thật (embedding + hybrid search) ngay từ đầu thay vì nối LLM trước với ngữ cảnh đơn giản.
  ...
  - 168 test backend (thêm ~50 test mới, toàn bộ mock OpenAI SDK — không gọi API thật trong CI) + 114 test frontend, lint/build sạch. **Chưa chạy `embed_knowledge.py`/tạo đề thật qua OpenAI** (tốn tiền thật) — để chủ dự án tự bấm chạy sau khi review code.
- **Spike chất lượng chuyển về đây:** sinh thử 3 dạng khó nhất (phát âm, trọng âm, đọc hiểu) với prompt ràng buộc chuẩn đã chốt; tiêu chí ≥70% câu chấp nhận không sửa. Dạng nào rớt → giữ chạy bằng ngân hàng câu hỏi/mock, lùi sinh tự động của dạng đó. **Còn lại:** cần chạy `embed_knowledge.py` thật + UAT với giáo viên trước khi đánh giá được tiêu chí này.
